...
class MemristorModel:
    ...
    def load_rgb_csv(path):
        ...
        if arr.ndim == 2 and arr.shape[1] >= 3:
            return {'r': arr[:, 0].astype(float), 'g': arr[:, 1].astype(float), 'b': arr[:, 2].astype(float)}
    ...
    def load_ltp_ltd_csv(path, ltp_count=31):
        ...
        for i, name in enumerate(cols):
            ...
            if n in ('b', 'blue'):
                ...
                idx_map['r'] = i
        # if mapping incomplete and there are at least 3 columns, assume order is b,g,r per your note
        if set(idx_map.keys()) != {'b', 'g', 'r'}:
            # fallback: assume first three columns correspond to b,g,r
            idx_map = {'b': 0, 'g': 1, 'r': 2}

        # load numeric data from data_lines robustly (support scientific notation and extra tokens)
        import re
        ...
        for line in data_lines:
            ...
            if len(nums_str) < 3:
                # try splitting by comma then extracting numbers per field
                ...
                nums_str = nums_try
            try:
                # enforce exactly as many columns as header (take first N numeric tokens)
                needed = len(cols)
        ...
        # map columns to r,g,b according to idx_map (file is b,g,r per your note)
        b = arr2[:, idx_map['b']].astype(float)
        g = arr2[:, idx_map['g']].astype(float)
        r = arr2[:, idx_map['r']].astype(float)

        # split into ltp and ltd by ltp_count
        def split_vals(v):
    ...
    def set_color_curves(self, ltp_rgb: dict, ltd_rgb: dict = None):
        ...
        ltp_rgb: dict with keys 'r','g','b' -> 1D numpy arrays
        ltd_rgb: same structure for depression curves (optional)
        """
    ...
    def pulses_for_conductance_change(self, current_g, target_g, color='r', up=True):
        ...
        Returns integer pulse count (positive means apply that many pulses in given direction).
        """
        curve = self.get_color_curve(color, up=up)
        # find indices of nearest values
        idx_cur = int((np.abs(curve - current_g)).argmin())
        # find minimum steps forward (positive pulses) to reach >= target magnitude
        # we handle direction by forward indexing
        # if target_g is outside curve range, clip
    ...
    def apply_weight_change(self, g_p, g_n, delta_w, color='r', mode='bidirectional'):
        ...
        Strategy:
        - bidirectional: if delta_w > 0, increase G+ by mapping delta to pulses on LTP curve for that color;
                       if delta_w < 0, increase G- similarly (so W = G+ - G- changes sign accordingly).
        - unidirectional: use only G+ for potentiation and use LTD on G+ for depression (or other device-specific scheme).

        Returns updated (g_p_new, g_n_new) and pulses applied (p_p, p_n).
        """
        # Simple mapping: map delta_w (assumed normalized in [-1,1]) to delta conductance
        # Use g_ref midpoint
        g_ref = (self.g_min + self.g_max) / 2.0
        # map normalized delta to absolute conductance change
        ...
        if mode == 'bidirectional':
            if delta_w > 0:
                ...
                # potentiate G- (increase negativity)
            ...
            # unidirectional: use G+ only; positive delta -> LTP on G+, negative delta -> LTD on G+
            if delta_w > 0:
                target_g_p = float(np.clip(g_p + desired_delta_g, self.g_min, self.g_max))
                p = self.pulses_for_conductance_change(g_p, target_g_p, color=color, up=True)
                g_p_new = self.apply_pulses(g_p, p, polarity='pos')
    ...
    def apply_weight_matrix_changes(self, Gp, Gn, deltaW, color='r', mode='bidirectional'):
        ...
        Gp, Gn are numpy arrays; deltaW same shape (signed, normalized -1..1). Returns new Gp,Gn and pulses arrays.
        """
        Gp_new = Gp.copy()
        Gn_new = Gn.copy()
        p_p = np.zeros_like(Gp, dtype=int)
        p_n = np.zeros_like(Gn, dtype=int)
        # determine per-element color mapping if caller doesn't specify a fixed color
            ...
            for i in range(rows):
                for j in range(cols):
                    if strategy == 'round_robin':
                        # cycle r,g,b across elements
                        idx = (i * cols + j) % 3
                        col = ['r','g','b'][idx]
                    elif strategy == 'blocks':
                        # split input dimension (cols) into 3 blocks
    ...
    def pulses_for_delta(self, delta_g, polarity='pos'):
        ...
        polarity: 'pos' -> use LTP (increase), 'neg' -> use LTD (decrease)
        """
        curve = self.ltp if polarity == 'pos' else self.ltd
        # compute differences between successive pulses
        diffs = np.diff(curve, prepend=curve[0])
        if delta_g == 0:
            return 0
        target = abs(delta_g)
        # pulses needed is min k such that cumulative diffs >= target
    ...
    def apply_pulses(self, g, pulses, polarity='pos'):
        ...
        pulses: number of pulses to apply
        polarity: 'pos' increases (LTP) else decreases (LTD)
        """
        curve = self.ltp if polarity == 'pos' else self.ltd
    ...
    def map_weight_to_conductances(self, w, g_ref=5e-5):
        ...
        Strategy: start from reference conductance and move G+ or G-.
        """
        # Keep devices within [g_min, g_max]
        half = max(self.g_min, min(self.g_max, g_ref))
        if w >= 0:
            g_p = self._synth_curve(len(self.ltp))[-1] * (w / (1.0 + w))
